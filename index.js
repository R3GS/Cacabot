...
function getDestinResponse(message) {
    const cible = message.mentions.users.first();

    const base = cible
        ? `Hmmm, le destin de ${cible} est...`
        : "Hmmm, ton destin est...";
...
client.on('messageCreate', async (message) => {
    ...
    // !destin — nécessite l'objet message pour les mentions
    if (response?.needsDestin) {
        return message.reply(getDestinResponse(message));
    }
